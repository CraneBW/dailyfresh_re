...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            subject='宁大生鲜%s'%order_id,
            return_url='http://127.0.0.1:8000/order/return',
...
class CheckPayView(View):
    ...
    def post(self, request):
        ...
        alipay = AliPay(
            appid="2021000147698109",  # 应用id
            app_notify_url='http://127.0.0.1:8000/order/notify',  # 默认回调url
            app_private_key_path=os.path.join(settings.BASE_DIR, 'apps/order/app_private_key.pem'),
        ...
        while True:
            try:
                ...
                print(f"支付宝响应: {response}")  # 记录完整响应内容
            except Exception as e:
                import logging
                logger = logging.getLogger('django')
                logger.error(f"支付宝交易查询失败: {str(e)}")
                return JsonResponse({'res': 4, 'errmsg': '支付状态查询失败'})
            # response = {
...
class AlipayReturnView(View):
    ...
    def get(self, request):
        print("get_AlipayReturnView" + str(request.GET))
        # 验证支付宝返回的参数
        alipay = AliPay(
            appid="2021000147698109",
            app_notify_url = None,
            app_private_key_path=os.path.join(settings.BASE_DIR, 'apps/order/app_private_key.pem'),
            alipay_public_key_path=os.path.join(settings.BASE_DIR, 'apps/order/alipay_public_key.pem'),
            sign_type="RSA2",
        ...
        if alipay.verify(params, sign):
            ...
            try:
                ...
                order.trade_no = trade_no
                order.order_status = 4  # 已支付，待评价
                order.save()
            except OrderInfo.DoesNotExist:
                pass
            # 跳转到订单详情页或成功页面
            print('# 跳转到订单详情页或成功页面')
            return HttpResponseRedirect(reverse('user:order', args=[1]))  # 跳转到订单列表页
        else:
...
class AlipayNotifyView(View):
    """支付宝异步通知（服务器间通信）"""    
    def post(self, request):
        print("post_AlipayNotifyView" + str(request.POST))
        alipay = AliPay(
            appid="2021000147698109",
            app_notify_url=None,
            app_private_key_path=os.path.join(settings.BASE_DIR, 'apps/order/app_private_key.pem'),
        ...
        if alipay.verify(data, sign):
            # 处理订单状态更新（逻辑与 AlipayReturnView 类似）
